...
这一节我们要讲解的 TypeScript 内置工具类型，包括了`访问性修饰`、`结构`、`集合`以及`模式匹配`工具类型这四种，我们会讲解它们的具体实现，也就是使用了哪些类型操作以及操作背后的原理，再思考它们存在哪些不足，存在的扩展方向有哪些。
...
![image-20240415134535307](https://jiny127.oss-cn-hangzhou.aliyuncs.com/typora/image-20240415134535307.png)
